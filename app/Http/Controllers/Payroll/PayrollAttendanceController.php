    ...
    public function getEmployeeAttendance($user_id,$from_date,$to_date,$payroll_period){
        ...
        foreach($date_range as $k => $date_r){
            ...
            if($employee_schedule && $time_in_data && $time_out_data){
                ...
                if ($if_leave) {
                    if(
                        $if_leave == 'VL Second Shift Without-Pay' 
                        || $if_leave == 'VL Second Shift With-Pay'
                    ...
                    if ($check_if_holiday) {
                        ...
                        if($if_leave){
                            if($if_leave == 'VL Without-Pay'){
                                ...
                                $sl += $check_leave_count_sl;
                            }else if($if_leave == 'VL First Shift Without-Pay' || $if_leave == 'VL Second Shift Without-Pay' || $if_leave == 'VL First Shift With-Pay' || $if_leave == 'VL Second Shift With-Pay'){
                        ...
                        if ($time_out_data == null && empty($if_leave)) {
                            if(empty($if_has_ob) && empty($if_has_wfh)){ 
                                $is_absent = 'Absent';
                            }
                        }else{
                            if($if_leave == 'VL Without-Pay'){
        ...
        if($emp->level == '2' || $emp->level == '3' || $emp->level == '4' || $emp->level == '5'){
            $lates = 0;
            $undertimes = 0;
            $night_diff_hours = 0;
        }
